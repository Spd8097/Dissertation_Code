...
def model_eval (ml_model):
    ...
    # dataframe of classified outcomes with its probability
...
def shap_plot(plot_model):
    ...
    shap.summary_plot(shap_values, df[predictors].values, plot_type="bar", feature_names=predictors)
...
gp.connect()
df_plot= pd.read_sql_query(f'SELECT * FROM {schema}.plot_1', con=gp)
gp.close()


df_footwear=df_plot[df_plot['product_desc'].str.contains('shoes|cleats|footwear', case=False, regex=True)]

df_footwear1=df_footwear.nlargest(5, ['avg_return_rate'])

import matplotlib.pyplot as mplt
from importlib import reload
mplt=reload(mplt)

x= np.array(df_footwear1['product_desc'])
y= np.array(df_footwear1['avg_return_rate'])
mplt.bar(x,y, color='maroon', width=0.4)
mplt.xlabel('Footwears')
mplt.ylabel("Highest average return rates")
mplt.title('Footwears with highest average return rate')
my_xticks=['Golf Shoes','Men AthleticFootwear', 'Men Footwear', 'Trans Golf Shoes', 'Snow Shoes']
mplt.xticks(x,my_xticks)
mplt.show()

df_brands=df_plot[df_plot['product_desc'].str.contains('Adidas', case=False, regex=True)]
#df_brands=df_plot[['product_desc', 'tot_quantity']]
df_brands1=df_brands.nlargest(5, ['avg_return_rate'])


x= np.array(df_brands1['product_desc'])
print(x)
y= np.array(df_brands1['avg_return_rate'])
y1= np.array(df_brands1['avg_ecom_rate'])
#y2= np.array(df_brands1['avg_quantity'])

mplt.bar(x,y, color='green', width=0.4)
mplt.bar(x,y1, color='blue', width=0.4)
#mplt.bar(x,y2, color='green', width=0.4)

mplt.xlabel('Products')
mplt.ylabel("Units Sold")
mplt.title('Products with highest Sales')
mplt.legend(['Return rate', 'Online Sales rate'])
my_xticks=['Non-merchanise items', 'Genaccess-Outdrequip', 'Third Part Non-Merchandise', 'Athletics Socks', 'Nike Mens Atheletic Footwear']
mplt.xticks(x,my_xticks)
mplt.xticks(rotation=5)
mplt.show()
